...
async function seedDatabase() {
  ...
  if (existing.length === 0) {
    ...
    await storage.createGame({
      stickerId: s3.id,
      question:
        "If you have a bowl with six apples and you take away three, how many do you have?",
      answer: "3",
      type: "choice",
      options: ["6", "3", "0", "9"],
    ...
    const s4 = await storage.createSticker({
      ...
      imageUrl:
        "/memes/st,small,507x507-pad,600x600,f8f8f8.u3_1766654814066.jpg",
      category: "Comedy",
    });
    await storage.createGame({
      stickerId: s4.id,
    ...
    const s6 = await storage.createSticker({
      ...
      imageUrl:
        "/memes/bg,f8f8f8-flat,750x,075,f-pad,750x1000,f8f8f8_1766654814067.jpg",
      category: "Reaction",
    });
    await storage.createGame({
      stickerId: s6.id,
      question:
        "A train travels at 60 mph. After 2 hours, how far has it traveled?",
      answer: "120",
      type: "text",
    ...
    const s8 = await storage.createSticker({
      ...
      imageUrl:
        "/memes/3ae325ca-ff88-484b-a553-f7bf8ed27c43_(1)_1766654814064.jpg",
      category: "Brain Teaser",
    });
    await storage.createGame({
      stickerId: s8.id,
...
export async function registerRoutes(
  ...
  app.get(api.games.getBySticker.path, async (req, res) => {
    ...
    if (!game) {
      return res
        .status(404)
        .json({ message: "Game not found for this sticker" });
    }
  ...
  app.post(api.games.verify.path, async (req, res) => {
    try {
      ...
      const isCorrect =
        game.answer.toLowerCase().trim() === answer.toLowerCase().trim();
  ...
  app.get(api.download.sticker.path, async (req, res) => {
    try {
      ...
      if (isLocalPath) {
        ...
        if (!fs.existsSync(filePath)) {
          return res
            ...
            .json({
              message:
                "Sticker image not found. Please add the image to /apps/web/public/memes/",
            });
        }
      ...
      const ext = sticker.imageUrl.includes(".png")
        ? ".png"
        : sticker.imageUrl.includes(".gif")
          ? ".gif"
          : sticker.imageUrl.includes(".webp")
            ? ".webp"
            : ".jpg";
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${sticker.name.replace(/\s+/g, "_")}${ext}"`,
      );
